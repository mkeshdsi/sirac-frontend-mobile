...
import { cadastrarTvr } from '@/services/apiResources';
...
const schema = yup.object({
  ...
  password: yup.string().min(6, 'A password deve ter pelo menos 6 caracteres').required('Password é obrigatória'),
  bi_frente: yup.string().optional(),
  bi_verso: yup.string().optional(),
});

type TvrForm = yup.InferType<typeof schema>;
...
export const TvrDataFormScreen = ({ navigation }: any) => {
  ...
  const { control, handleSubmit, setValue, watch, formState: { errors } } = useForm<TvrForm>({
    resolver: yupResolver(schema),
    defaultValues: { nome: '', email: '', msisdn: '', bi: '', nuit: '', password: '', bi_frente: '', bi_verso: '' },
  });
  ...
  const onSubmit = async (data: TvrForm) => {
    setLoading(true);
    try {
      const result = await cadastrarTvr(data);
      ...
      setShowErrorModal({
        ...
        message: apiMessage || 'Ocorreu um erro no servidor. Verifique se os dados já não existem (BI, Contacto, Email).',
      });
    } finally {
      setLoading(false);
    }
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={styles.header}>
          ...
          <View style={styles.headerTitleContainer}>
            <Text style={styles.headerTitle}>Novo TVR</Text>
            <Text style={styles.headerSubtitle}>Preencha os dados do técnico de venda</Text>
          </View>
          <View style={styles.stepPill}>
            <Text style={styles.stepPillText}>4 seccoes</Text>
          </View>
        </View>

        <View style={styles.progressBar}>
          <View style={[styles.progressFill, { width: loading ? '100%' : '70%' }]} />
        </View>
        ...
        <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <SectionCard icon="card-outline" iconBg={COLORS.secondaryLight} iconColor="#c49b00" title="Identificação">
          ...
          <SectionCard icon="key-outline" iconBg="#e3f2fd" iconColor="#1976d2" title="Acesso">
            <Controller control={control} name="password" render={({ field: { onChange, value } }) => (
              <Input label="Password inicial *" placeholder="Defina a password do TVR" secureTextEntry
                value={value} onChangeText={onChange} error={errors.password?.message}
                leftIcon={<Ionicons name="lock-closed-outline" size={18} color={COLORS.textSecondary} />} />
            )} />
            <View style={styles.infoBox}>
              <Ionicons name="information-circle-outline" size={18} color={COLORS.primary} />
              <Text style={styles.infoBoxText}>Informe esta password ao TVR para o primeiro acesso.</Text>
            </View>
          </SectionCard>

          <SectionCard icon="images-outline" iconBg="#f3e8ff" iconColor="#7c3aed" title="Documentos Anexos">
            <Text style={styles.sectionDescription}>
              Anexe imagens claras da frente e do verso do BI, se disponíveis.
            </Text>
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  <Text style={styles.submitBtnText}>Salvar TVR</Text>
                </>
              )}
            </LinearGradient>
          </TouchableOpacity>
      ...
      <Modal visible={showSuccessModal} transparent animationType="fade">
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalMessage}>TVR registado com sucesso.</Text>
            <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  stepPillText: { fontSize: 11, color: COLORS.primary, fontWeight: '700' },
  ...
  sectionDescription: { fontSize: 13, color: COLORS.textSecondary, marginBottom: 14, lineHeight: 19 },
  infoBox: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 10,
  ...
  infoBoxText: { flex: 1, fontSize: 13, lineHeight: 18, color: COLORS.primary, fontWeight: '600' },
  ...
  docButtonSuccess: { borderColor: 'transparent' },
  docButtonInner: {
    flexDirection: 'row',
    alignItems: 'center',
    paddingVertical: 13,
  ...
  docButtonText: { flex: 1, fontSize: 14, fontWeight: '600', color: COLORS.primary, letterSpacing: -0.1 },
  ...
  submitBtnInner: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 16 },
  ...
  modalTitle: { fontSize: 22, fontWeight: '800', color: COLORS.text, letterSpacing: -0.4, marginBottom: 8 },
  modalMessage: { fontSize: 14, color: COLORS.textSecondary, textAlign: 'center', lineHeight: 21, marginBottom: 24 },
  modalBtn: {
    width: '100%',
  ...
  modalBtnText: { fontSize: 15, fontWeight: '700', color: 'white', letterSpacing: -0.1 },
